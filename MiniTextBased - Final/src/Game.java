            ...
            displayStats();
                ...
                if (!map.get(returnCurrentRoomID()).getPuzzles().isEmpty()) {
                    ...
                    if (str.equalsIgnoreCase("solve puzzle")) {
                        ...
                        for (int attempts = 3; attempts > 0; attempts--) {
                            if (!map.get(returnCurrentRoomID()).getPuzzles().isEmpty()) {
                                if (!str2.equalsIgnoreCase(
                                        map.get(returnCurrentRoomID()).getPuzzles().get(0).getAnswer())) {
                                    str2 = input.nextLine();
                                    solvePuzzle(str2);
                                } else {
                                    str2 = input.nextLine();
                        ...
                        System.out.println("Inspect the puzzle again.");
                    ;
                }
                ...
                if (str.equalsIgnoreCase("attack")) {
                    ...
                    while (!(p1.getHealth() <= 0)
                            && !(map.get(returnCurrentRoomID()).getMonsters().get(0).getHealth() <= 0)) { // starts the
                                                                                                          // fight
                        ...
                        if (str2.toLowerCase().contains("pickup") || // differnt inputs for item interactions
                                str2.toLowerCase().contains("drop") || str2.toLowerCase().contains("examine")
                                || str2.toLowerCase().contains("equip") || str2.toLowerCase().contains("unequip")
                                || str2.toLowerCase().contains("attack") || str2.toLowerCase().contains("heal")) {
...
//        }
    ...
    public static void readItemsFile() throws FileNotFoundException { // method to read item.txt and put objects into
        ...
        for (int i = 0; i <= 14; i++) { // loops 6 times
            ...
            if (s.next().equalsIgnoreCase("roomID:"))
                item.setRoomId(parseInt(s.next(rx)));
            if (s.next().equalsIgnoreCase("name:"))
                item.setItemName((s.next(rx)));
            if (s.next().equalsIgnoreCase("description:"))
                item.setDescription((s.next(rx)));
            if (s.next().equalsIgnoreCase("attackpoints:"))
                item.setAttackPoints(parseInt(s.next(rx)));
            if (s.next().equalsIgnoreCase("HealthRestored:"))
                item.setHpRestored((parseInt(s.next(rx))));
            if (s.next().equalsIgnoreCase("defense:"))
    ...
    public static void readPuzzlesFile() throws FileNotFoundException { // method to read item.txt and put objects into
        ...
        for (int i = 0; i <= 6; i++) { // loops 6 times
            ...
            if (s.next().equalsIgnoreCase("roomID:"))
                puzzle.setRoomID(parseInt(s.next(rx)));
            if (s.next().equalsIgnoreCase("question:"))
                puzzle.setQuestion(s.next(rx));
            if (s.next().equalsIgnoreCase("answer:"))
                puzzle.setAnswer((s.next(rx)));
            if (s.next().equalsIgnoreCase("hint:"))
                puzzle.setHint((s.next(rx)));
    ...
    public static void readMonsterFile() throws FileNotFoundException { // method to read item.txt and put objects into
        ...
        for (int i = 0; i <= 13; i++) { // loops 6 times
            ...
            if (s.next().equalsIgnoreCase("monsterID:"))
                monster.setMonsterID(parseInt(s.next(rx)));
            if (s.next().equalsIgnoreCase("name:"))
                monster.setName(s.next(rx));
            if (s.next().equalsIgnoreCase("health:"))
    ...
    private static void pickupItem(String itemName) { // may or may not work
        for (int i = 0; i < map.get(returnCurrentRoomID()).getRoomInventory().size(); i++) {
            if (itemName.equalsIgnoreCase(map.get(returnCurrentRoomID()).getRoomInventory().get(i).getItemName())) { // checks
                                                                                                                     // item
                                                                                                                     // name
                p1.getInventory().add(map.get(returnCurrentRoomID()).getRoomInventory().get(i)); // adds item to player
                                                                                                 // inventory
                System.out.println("You have picked up "
                        + (map.get(returnCurrentRoomID()).getRoomInventory().get(i).getItemName()));
        ...
        for (int i = 0; i <= p1.getInventory().size(); i++) {
            if (!p1.getInventory().isEmpty()) {
                if (itemName.equalsIgnoreCase(p1.getInventory().get(i).getItemName())) { // checks item name
                    map.get(returnCurrentRoomID()).getRoomInventory().add(p1.getInventory().get(i)); // adds item to
                                                                                                     // room inventory
    ...
    private static void attackMonster(String monsterName) { // subtracts player attack points from monster
        if (map.get(returnCurrentRoomID()).getMonsters().get(0).getName().equalsIgnoreCase(monsterName)) {
            map.get(returnCurrentRoomID()).getMonsters().get(0).setHealth(
